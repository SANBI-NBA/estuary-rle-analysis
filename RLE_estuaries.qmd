...
# calculate ongoing decline and fishing increases
 ...
 # D3 fishing pressure assessment  
    mutate(D3f_24 = case_when( prp24_fp_vh >= 0.9 ~ "CR", 
                              prp24_fp_vh >= 0.7 | prp24_fp_hvh >=0.9 ~ "EN",
                              prp24_fp_vh >= 0.5 | prp24_fp_hvh >=0.7 | prp24_fp_mhvh >= 0.9 ~ "VU",
                              TRUE ~ "LC")) %>% 
    mutate(D3f_24 = factor(D3f_24, levels = c("LC","VU", "EN", "CR"), ordered = TRUE)) %>%
    mutate(D3f_18 = case_when( prp18_fp_vh >= 0.9 ~ "CR", 
                              prp18_fp_vh >= 0.7 | prp18_fp_hvh >=0.9 ~ "EN",
                              prp18_fp_vh >= 0.5 | prp18_fp_hvh >=0.7 | prp18_fp_mhvh >= 0.9 ~ "VU",
                              TRUE ~ "LC")) %>% 
     mutate(D3f_18 = factor(D3f_18, levels = c("LC","VU", "EN", "CR"), ordered = TRUE)) %>%
       # D3 PES 
   ...
   relocate( A2b_18, A3_18, B3_18, D3f_18, D3_18,  A2b_24, A3_24,   B3_24, D3f_24, D3_24 , 
            .after = prplost9040_9018) %>%
  # overall RLE outcome USING  A2b A3 B1 B3 D3
     rowwise() %>%
   # FINAL RLE BUT EXCLUDE B1aii and use B3 rather
    mutate(RLE18 = max(c_across(all_of(c( "D3f_18", "D3_18",  "B3_18", "A2b_18", "A3_18"))), na.rm = TRUE)) %>%
    mutate(RLE24 = max(c_across(all_of(c( "D3f_24",  "D3_24",  "B3_24", "A2b_24", "A3_24"))), na.rm = TRUE)) %>%
     ungroup() %>%
   rowwise() %>%
  mutate(trigger18 = paste(c("A2b_18", "A3_18", "B3_18", "D3f_18", "D3_18")
                           [c_across(A2b_18:D3_18) == RLE18], collapse = ", ")) %>%
  mutate(trigger24 = paste(c("A2b_24", "A3_24", "B3_24","D3f_24",   "D3_24")
